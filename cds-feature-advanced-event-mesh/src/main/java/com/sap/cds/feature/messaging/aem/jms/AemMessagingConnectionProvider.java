...
public class AemMessagingConnectionProvider extends BrokerConnectionProvider {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(AemMessagingConnectionProvider.class);
  ...
  public AemMessagingConnectionProvider(ServiceBinding binding) {
    ...
    this.destination =
        DefaultHttpDestination.fromDestination(
  ...
  protected BrokerConnection createBrokerConnection(
    ...
    // see
    // https://solace.community/discussion/1677/how-oauth-can-be-used-with-apache-qpid-jms-2-0-amqp
    logger.debug(
        "Retrieving credentials for Basic Auth from	service binding '{}'", binding.getName().get());

    final BiFunction<Connection, URI, Object> tokenExtension =
        new BiFunction<>() {
          @Override
    ...
    logger.debug(
        "Creating connection factory fo service binding '{}'", this.binding.getName().get());
    // the password is going to be replaced by the token
    JmsConnectionFactory factory =
        new JmsConnectionFactory(
            destination.get("vpn", String.class).get(),
            "token",
            destination.get(DestinationProperty.URI).get());
